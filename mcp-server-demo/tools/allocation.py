...
@mcp.tool()
async def create_resource_allocation(resource_name: str, work_name:str, task_name:str, pf_login_cert: Optional[str] = None) -> str:
    """
    Create an Allocation under a task for a resource or assign a task to a resource
    """
    work_details = Session.work_cache.get(work_name, {})
    # if not work_details:
    ...
    Session.login_cert = pf_login_cert
